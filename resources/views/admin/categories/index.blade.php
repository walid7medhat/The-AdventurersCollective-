                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    @permission('create_categories')
                       <a href="{{url('/')}}/admin/categories/create" class="btn btn-info text-white float-right">@lang('site.create new')</a>
                    @endpermission
                  </div>    
                </div>
                <div class="row">
                  ...
                  <tbody>
                      @foreach($categories as $i=>$category)
                             <tr class="delete{{$category->id}}">
                                 <td><input type="checkbox" value="{{$category->id}}" name="delete[]"/></td>
                                 <td>{{$category->title}}</td>
                                 <td>{{$category->created_at->year}}-{{$category->created_at->month}}-{{$category->created_at->day}}</td>
                                 <td>
                                 @permission('delete_categories')
                                   <a class="btn btn-danger delete" data-id="{{$category->id}}"><i class="fa fa-trash text-white"></i></a>
                                 @endpermission
                                 @permission('update_categories')
                                 <a href="{{url('/')}}/admin/categories/{{$category->id}}/edit" class="btn btn-primary update" data-id="{{$category->id}}"><i class="fa fa-pen text-white"></i></a> 
                                 @endpermission
        ...
        if(searchIDs.length >0){
          ...
          cancelButtonText:'@lang("site.No, cancel!")',
              ...
              if (result.isConfirmed) {
                  $.ajax({
                  url:"{{ url('/admin/categories/delete_all')}}",
                  type:'post',
