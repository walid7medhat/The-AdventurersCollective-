  ...
  <div class="content-wrapper">
       ...
       <section class="content-header">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6">
                <h1>@lang('site.gallery')</h1>
              </div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item"><a href="{{url('/')}}">@lang('site.dashboard')</a></li>
                  <li class="breadcrumb-item active">@lang('site.gallery')</li>
                </ol>
              </div>
            </div>
          </div><!-- /.container-fluid -->
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                  ...
                  <div class="col-md-6">
                      <a href="{{url('/')}}/admin/setting/slidear/create" class="btn btn-info text-white float-right">@lang('site.create new')</a>
                  </div>    
                </div>
                <div class="row">
                     <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped table-hover">
                  ...
                  <tbody>
                      @foreach($slidears as $i=>$slidear)
                             <tr class="delete{{$slidear->id}}">
                                 <td><input type="checkbox" value="{{$slidear->id}}" name="delete[]"/></td>
                                 <td><img src="{{url('/')}}/{{$slidear->image}}" style="max-width:100px"/></td>
                                 <td>{{$slidear->title}}</td>
                                 <td>{{$slidear->description}}</td>
                                 <td>{{$slidear->created_at->year}}-{{$slidear->created_at->month}}-{{$slidear->created_at->day}}</td>
                                 <td>
                                   <a class="btn btn-danger delete" data-id="{{$slidear->id}}"><i class="fa fa-trash text-white"></i></a>
                                   <a href="{{url('/')}}/admin/setting/slidear/{{$slidear->id}}/edit" class="btn btn-primary update" data-id="{{$slidear->id}}"><i class="fa fa-pen text-white"></i></a>                                  
                                </td>
                            </tr>
                      @endforeach
                  </tbody>
        ...
        if(searchIDs.length >0){
          ...
          cancelButtonText:'@lang("site.No, cancel!")',
              ...
              if (result.isConfirmed) {
                  $.ajax({
                  url:"{{ url( '/admin/setting/slidear/delete_all')}}",
                  type:'post',
    ...
    $('.delete').on('click',function(){
            ...
            Swal.fire({
              ...
              cancelButtonText:'@lang("site.No, cancel!")',
                  ...
                  if (result.isConfirmed) {
                      $.ajax({
                      url:"{{ url('/admin/setting/slidear') . '/' }}" + id,
                      type:'delete',
